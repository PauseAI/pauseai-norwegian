...
const norwegians: Person[] = [je];



//export async function GET({ fetch }) {
export async function GET() {
	const url = 'https://pauseai.info/api/people'
	const response = await fetch(url);
	if (!response.ok) {
		throw new Error('Failed to fetch data from PauseAI.info')
	}
	...
	const data = await response.json()
	const out: Person[] = data.recordsnpm 
		.map(recordToPerson)
		.filter((p: Person) => p.image && !p.privacy)
		// Shuffle the array, although not truly random
		.sort(() => 0.5 - Math.random())*/
